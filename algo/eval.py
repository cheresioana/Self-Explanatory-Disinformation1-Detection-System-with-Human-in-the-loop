...
def main(file_name):
    ...
    for i in range(4, 9):
        ...
        all_nodes, matches = load_structure_file("results/full_result_" + str(index) + ".json")
        ...
        print(
            f"Count foun narrative {count_found_narrative} ot of which correct {found_narrative_correct}, TX {count_found_te}, out of which has parent {has_parent} out of which correct {te_correct}")
        acc.append(accuracy)
        res.append(cm)
        # break
...
if __name__ == '__main__':
    main('data/work_data/validation_df.csv')
    # main('data/test_data/sbert_emb/veridica_test_emb2.csv')
